...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json<ApiResponse<WeeklySignalEntry[]>>(
      { data: data as WeeklySignalEntry[], timestamp: new Date().toISOString() },
      { status: 200 }
    );
  } catch (error) {
    return NextResponse.json<ApiResponse>(
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const validation = upsertSignalEntrySchema.safeParse(body);
    ...
    const { signal_id, week_start, current_count } = validation.data;

    const { data, error } = await supabase
      .from('weekly_signal_entries')
      .upsert(
        { signal_id, week_start, current_count, updated_at: new Date().toISOString() },
        { onConflict: 'signal_id,week_start' }
      )
      .select()
      .single();
